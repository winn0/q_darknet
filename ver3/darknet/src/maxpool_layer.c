...
maxpool_layer make_quantized_maxpool_layer(int batch, int h, int w, int c, int size, int stride_x, int stride_y, int padding, int maxpool_depth, int out_channels, int antialiasing, int avgpool, int train,int quantization_type)
{
    maxpool_layer l = { (LAYER_TYPE)0 };
    l.avgpool = avgpool;
    if (avgpool) l.type = LOCAL_AVGPOOL;
    ...
    if(l.quantization_type){
        l.quantized_output_uint8 = (unsigned char*)xcalloc(output_size, sizeof(char));
        //l.output = (float*)xcalloc(output_size, sizeof(float));
    }
    else{
        l.output = (float*)xcalloc(output_size, sizeof(float));
                    ...
                    for (j = 0; j < l.w; ++j) {
                        for (g = 0; g < l.out_c; ++g)
                        {
                            ...
                            for (k = g; k < l.c; k += l.out_c)
                            {
                                ...
                                unsigned char val = state.quantized_input[in_index];

                                max_i = (val > uint8_min) ? in_index : max_i;
                                uint8_min = (val > uint8_min) ? val : uint8_min;
                            }
                            l.output[out_index] = uint8_min;
                            ...
                            for (n = 0; n < l.size; ++n) {
                                for (m = 0; m < l.size; ++m) {
                                    ...
                                    int valid = (cur_h >= 0 && cur_h < l.h &&
                                        cur_w >= 0 && cur_w < l.w);
                                    unsigned char val = (valid != 0) ? state.input[index] : -FLT_MAX;
