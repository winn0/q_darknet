    ...
    ACTIVATION activation, int train,int quantization_type)
{
    ...
    l.input_layers = input_layers;
    l.input_sizes = input_sizes;
    l.layers_output_uint8 = layers_output_uint8;
    l.layers_delta = layers_delta;
    l.weights_type = weights_type;
    l.weights_normalization = weights_normalization;
    l.learning_rate_scale = 1;  // not necessary
    ...
    if(l.quantization_type){
       l.quantized_output = (int*)xcalloc(batch*l.outputs, sizeof(int));
       l.quantized_output_uint8 = (unsigned char*)xcalloc(l.outputs * batch, sizeof(char));
       l.output = (float*)xcalloc(l.outputs * batch, sizeof(float));
...
void forward_shortcut_layer(const layer l, network_state state)
{
    ...
    // printf("l.n %d from_w %d l.w %d from_h %d l.h %d from_c %d l.c %d\n",l.n,from_w,l.w,from_h,l.h,from_c,l.c);
    if (l.nweights == 0 && l.n == 1 && from_w == l.w && from_h == l.h && from_c == l.c) {
        ...
        if (l.quantization_type){
            ...
            unsigned char *quantized_output_uint8_to_be_added = (unsigned char *)xcalloc(size, sizeof(unsigned char));
            #pragma omp parallel for
            for(i = 0; i < size; ++i){
           ...
           int temp_sum =0;
            ...
            conv_output_quantization(q_a,q_b,q_c,from_c,l.c,l.h,l.w,1,1,l.quantization_layer_scale,l.quantization_layer_zeropoint,l.quantization_per_channel_scale,l.quantization_per_channel_zeropoint,state.quantized_input_scale,state.quantized_input_zeropoint,quantized_output_uint8_to_be_added,l.biases);
            for(i = 0; i < 16; ++i){
    ...
    if (l.activation == SWISH) activate_array_swish(l.output, l.outputs*l.batch, l.activation_input, l.output);
        ...
        if(state.quantization_type){
            quantized_activate_array_cpu_custom(l.quantized_output_uint8, l.outputs*l.batch, l.activation,state.quantized_input_scale);
        }
        else{
