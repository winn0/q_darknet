...
layer parse_shortcut(list *options, size_params params, network net)
{
    ...
    unsigned char **layers_output_uinit8 = (unsigned char **)calloc(n, sizeof(unsigned char *));
    ...
    if (quantization_type){
        for (i = 0; i < n; ++i) {
            ...
            layers_output_uinit8[i] = params.net.layers[index].quantized_output_uint8;
...
#else
        if (workspace_size) {
            printf("workspace size :%d",workspace_size);
            net.workspace = (float*)xcalloc(1, workspace_size);
        }
#endif
...
{  
    ...
    if(quantization_type){
        ...
        printf("1.quantized_weights[0]:%d\n",l.quantized_weights[0]);
        printf("1.quantized_weights[1]:%d\n",l.quantized_weights[1]);
        printf("1.quantized_weights[2]:%d\n",l.quantized_weights[2]);
...
void load_quantized_convolutional_weights(layer l, FILE *fp, int quantization_type)  ///where is define?
{
    ...
    if (quantization_type){
        ...
        read_bytes = fread(l.quantization_per_channel_scale, sizeof(float), l.n, fp);
        printf("qconv_per_channel_scale[0]=%f\n",l.quantization_per_channel_scale[0]);
        ...
        read_bytes = fread(l.biases, sizeof(float), l.n, fp);
        printf("qconv_bias[0]=%f\n",l.biases[0]);
        //fortest
        for (int i ; i<l.n;i++){
            printf("scale for channel %d : %f\n",i,l.quantization_per_channel_scale[i]);
            printf("zeropoint for channel %d : %d\n",i,l.quantization_per_channel_zeropoint[i]);
        }
        if (read_bytes > 0 && read_bytes < l.n) printf("\n Warning: Unexpected end of wights-file! l.weights - l.index = %d \n", l.index);
        if (l.flipped) {
...
void load_quantized_shortcut_weights(layer l, FILE *fp)
{
    ...
    read_bytes = fread(l.quantized_weights, sizeof(char), num, fp);
    printf("qconv_weight[0]=%d\n",l.quantized_weights[0]);
    read_bytes = fread(l.quantization_per_channel_scale, sizeof(float), l.c, fp);
    printf("qconv_per_channel_scale[0]=%f\n",l.quantization_per_channel_scale[0]);
